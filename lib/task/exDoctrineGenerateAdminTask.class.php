 ...
 */
require_once(sprintf("%s/plugins/sfDoctrinePlugin/lib/task/sfDoctrineBaseTask.class.php", sfConfig::get('sf_symfony_lib_dir')));
/**
 * Generates a Doctrine admin module with a comfirm process.
 *
 * @package    symfony
 * @subpackage doctrine
 ...
 * @version    $Id
 */
class exDoctrineGenerateAdminTask extends sfDoctrineBaseTask
{
  /**
   * @see sfTask
   */
  protected function configure()
  {
    ...
    $this->addOptions(array(
      ...
      new sfCommandOption('theme', null, sfCommandOption::PARAMETER_REQUIRED, 'The theme name', 'admin-plus'),
    ...
    $this->name = 'generate-admin-plus';
    $this->briefDescription = 'Generates a Doctrine admin module with the process to confirm input values';

    $this->detailedDescription = <<<EOF
The [doctrine:generate-admin-plus|INFO] task generates a Doctrine admin module:

  [./symfony doctrine:generate-admin-plus frontend Article|INFO]
...
EOF
  ...
  protected function generateForRoute($arguments, $options)
  {
    ...
    if (!$arguments['route'] instanceof sfDoctrineRouteCollectionPlus)
    {
      throw new sfCommandException(sprintf('The route "%s" is not a Doctrine collection plus route.', $arguments['route_name']));
    }
  ...
  {
    if ($route instanceof sfDoctrineRouteCollectionPlus)
    {
