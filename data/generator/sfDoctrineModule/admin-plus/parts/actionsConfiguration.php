...
<?php unset($this->config['actions']) ?>
  ...
  {
    if (sfContext::getInstance()->getRouting()->hasRouteName('<?php echo $this->moduleName ?>_cfnew')) {
      return <?php echo $this->asPhp(isset($this->config['form']['actions']) ? $this->config['form']['actions'] : array('_delete' => null, '_list' => null, '_confirm' => null)) ?>;
    } else {
      return <?php echo $this->asPhp(isset($this->config['form']['actions']) ? $this->config['form']['actions'] : array('_delete' => null, '_list' => null, '_save'  => null, 'save_and_add' => null)) ?>;
    }
<?php unset($this->config['form']['actions']) ?>
  }

  public function getConfirmActions()
  {
   return <?php echo $this->asPhp(isset($this->config['confirm']['actions']) ? $this->config['confirm']['actions'] : array('_list' => null, '_back' => null, '_save' => null, '_save_and_add' => null)) ?>;
<?php unset($this->config['confirm']['actions']) ?>
  }
